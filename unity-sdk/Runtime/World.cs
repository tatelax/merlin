	...
	public abstract class World : IWorld, IDisposable
	{
		...
		public virtual void OnEntityCreated(Entity.Entity entity, bool entityCreatedFromNetworkMessage)
		{
			if (SimulationController.Instance.LogLevel == LogLevel.Verbose)
				Debug.Log("<color=#00ff00>[MerlinSDK] Creating entity with id " + entity.id + " in world " + GetType().Name + "</color>");
		...
		public virtual void OnEntityDestroyed(Entity.Entity entity, bool entityDestroyedFromNetworkMessage = false)
		{
			if (SimulationController.Instance.LogLevel == LogLevel.Verbose)
			{
				Debug.Log("<color=#00ff00>[MerlinSDK] " + $"{entity.id} was <b>DESTROYED</b> in world {GetType().Name} </color>");
			}
		...
		public virtual void OnComponentAddedToEntity(Entity.Entity entity, IComponent component)
		{
			if (SimulationController.Instance.LogLevel == LogLevel.Verbose)
			{
				Debug.Log("<color=#00ff00>[MerlinSDK] " + $"{component.GetType()} was <b>ADDED</b> to entity {entity.id} in world {GetType().Name} </color>");
			}
		...
		{
			if (SimulationController.Instance.LogLevel == LogLevel.Verbose)
				Debug.Log("<color=#00ff00>[MerlinSDK] " + $"{component.GetType()} was <b>REMOVED</b> from entity {entity.id} in world {GetType().Name} </color>");
		...
		{
			if (SimulationController.Instance.LogLevel == LogLevel.Verbose)
				Debug.Log("<color=#00ff00>[MerlinSDK] " + $"{component.GetType()} was <b>SET</b> on entity {entity.id} in world {GetType().Name} </color>");
